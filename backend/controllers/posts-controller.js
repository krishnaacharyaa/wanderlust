...
export const createPostHandler = async (req, res) => {
  try {
    ...
    if (!imageLinkRegex.test(imageLink)) {
      return res.status(HTTP_STATUS.BAD_REQUEST).json({ message: 'Image URL must end with .jpg, .jpeg, .webp, or .png' });
    }

    // Validation - check if categories array has more than 3 items
    if (categories.length > 3) {
      return res.status(HTTP_STATUS.BAD_REQUEST).json({ message: 'Please select up to three categories only.' });
    }
