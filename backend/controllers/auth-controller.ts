...
import { accessCookieOptions, refreshCookieOptions } from '../utils/cookie_options';
const { hash, compareSync } = bcrypt;
const { sign } = jwt;

//REGULAR EMAIL PASSWORD STRATEGY
//1.Sign Up
export const signUpWithEmail = async (req : Request, res: Response, _next : NextFunction) => {
  try {
...
export const openGoogleAuthWindow = (req : Request, res: Response, next : NextFunction) => {
...
//3.Sign In
export const signInWithGoogle = async (req : Request, res: Response, _next : NextFunction) => {
  const code = req.query.code;
  if (!code) {
    res.status(HTTP_STATUS.BAD_REQUEST).json({
      success: false,
...
export const openGithubAuthWindow = (_req : Request, res: Response, _next : NextFunction) => {
...
//2.Sign up
export const signUpWithGithub = async (req : Request, res: Response, next : NextFunction) => {
  const code = req.query.code;
  if (!code) {
    res.status(HTTP_STATUS.BAD_REQUEST).json({
      success: false,
...
//3.Sign In
export const signInWithGithub = async (req : Request, res: Response, next : NextFunction) => {
  const code = req.query.code;
  if (!code) {
    res.status(HTTP_STATUS.BAD_REQUEST).json({
      success: false,
...
//Sign Out
export const signOutUser = async (_req : Request, res: Response, _next : NextFunction) => {
  try {
